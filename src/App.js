...
class Fruitgame extends Component {
  state = {
    matchImageIndex: imagesList[Math.floor(Math.random() * imagesList.length)],
  ...
  randomImage = () => {
    ...
    this.setState({matchImageIndex: imagesList[randomNumber]})
  }
  ...
  filteredtbnl = tabId => {
    return imagesList.filter(each => each.category === tabId)
  }

  checkrandomImage = tbnlid => {
    const {id} = this.state.matchImageIndex
    if (id === tbnlid) {
      this.setState(
  ...
  render() {
    const {matchImageIndex, tabId, score, timer, gameOver} = this.state
    const matchImage = matchImageIndex.imageUrl
    const choosetbnlCatogery = this.filteredtbnl(tabId)
    if (timer === 0 || gameOver === true) this.clearTimer()
    ...
    return (
      <div className="bg-container">
          ...
          <div className="score-container">
            ...
            <div className="score-container">
              <img
                ...
                alt="timer"
              />
              <p className="score-color">{timer} sec</p>
            </div>
          </div>
        </div>
        ...
        {timer === 0 || gameOver === true ? (
          ...
          <div className="main-container">
            <img className="match-image" src={matchImage} alt="match" />
            <ul className="button-container">
              {tabsList.map(({tabId, displayText}) => (
                <li key={tabId}>
                  <button
                    onClick={() => this.changeTab(tabId)}
                    className={`button ${this.state.tabId === tabId ? 'active' : ''}`}
                  >
                    {displayText}
                  </button>
                </li>
            ...
            <ul className="list-container">
              {choosetbnlCatogery.map(({thumbnailUrl, id}) => (
                <li key={id}>
                  <img
                    onClick={() => this.checkrandomImage(id)}
